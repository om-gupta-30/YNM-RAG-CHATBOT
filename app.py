...
if api_key:
    ...
    logging.warning(
        "GEMINI_API_KEY not set. Gemini-dependent endpoints will be unavailable."
    )
...
def deduplicate_chunks(
    chunks_data: List[dict], similarity_threshold: float = 0.85
) -> List[dict]:
    if not chunks_data:
        return []
...
def _compute_confidence_for_block(
    ...
    # DEDUCTIONS
    primary_type = (
        (used_sources[0].source_type or used_sources[0].source) if used_sources else ""
    ) or ""
    if primary_type == "OCR":
        score -= 0.15
...
def _extract_figure_caption(chunk_text: str, target_id: Optional[str]) -> Optional[str]:
    ...
    pat = re.compile(
        rf"\bfig(?:ure)?\.?\s*{re.escape(ref)}\b\.?\s*(.*)$", re.IGNORECASE
    )
...
async def classify_intent(request: QuestionRequest):
    ...
    if info.get("intent") == INTENT_COMPARISON:
        ...
        for m in re.finditer(
            r"\b(fig(?:ure)?|illustration)\b\.?\s*(\d+(?:\.\d+)*)", q, re.IGNORECASE
        ):
...
@app.post(
    "/generate-chat-title", response_model=GenerateChatTitleResponse | ErrorResponse
)
...
async def expand_context(request: ContextExpandRequest):
    ...
    for cid in request.chunk_ids or []:
        if str(cid) in metadata:
            core_sources.append(
                _build_source_from_metadata_chunk(
                    str(cid), intent_info, include_raw_text=True
                )
            )
...
def _extract_matching_reference_line(
    text: str, intent: str, target_id: Optional[str]
) -> Optional[str]:
    """
...
def _extract_comparison_targets(
    ...
    if len(figs) >= 1 and len(tables) >= 1:
        return (
            None,
            None,
            "Invalid comparison. Table ↔ Figure comparisons are not supported.",
        )
    ...
    if len(pages) >= 2:
        return (
            None,
            None,
            "Invalid comparison. Only Table ↔ Table or Figure ↔ Figure comparisons are supported.",
        )
...
def _retrieve_one_item_by_ref(kind: str, target_id: str) -> Optional[dict]:
    ...
    if ref:
        if kind == "FIGURE_QUERY":
            ref_re = re.compile(
                rf"\bfig(?:ure)?\.?\s*{re.escape(ref)}\b", re.IGNORECASE
            )
            src_pri = {"image_ocr": 0, "vision": 1, "text": 2}
        else:
            ref_re = re.compile(
                rf"\b(?:table|tab)\.?\s*{re.escape(ref)}\b", re.IGNORECASE
            )
        ...
        def _table_score(item: dict) -> int:
            ...
            class_lines = sum(
                1 for ln in lines if re.search(r"\bCLASS\b", ln, re.IGNORECASE)
            )
            multi_col_lines = sum(
                1 for ln in lines if re.search(r"\s{2,}", ln) or "|" in ln or "\t" in ln
            )
        ...
        def _figure_score(item: dict) -> int:
            if kind != "FIGURE_QUERY":
                return 0
            line = (
                _extract_matching_reference_line(
                    item.get("text", "") or "", "FIGURE_QUERY", target_id
                )
                or ""
            )
            l = line.lower().strip()
...
def _build_source_from_metadata_chunk(
    ...
    preview_text = _extract_matching_reference_line(
        chunk_text, intent, intent_info.get("target_id")
    )
    if not preview_text:
    ...
    caption = (
        _extract_figure_caption(chunk_text, intent_info.get("target_id"))
        if intent == "FIGURE_QUERY"
        else None
    )
    ...
    for s in core_sources:
        ...
        if cid is not None:
            ...
            if prev_id in metadata:
                prev_src = _build_source_from_metadata_chunk(
                    prev_id, intent_info, include_raw_text=True
                )
            if next_id in metadata:
                next_src = _build_source_from_metadata_chunk(
                    next_id, intent_info, include_raw_text=True
                )
        ...
        out.append(
            ContextualSource(
                core_chunk=s,
                surrounding_context=SurroundingContext(
                    previous=prev_src, next=next_src
                ),
            )
        )
    return out
...
def _retrieve_chunks_by_intent(intent_info: dict, question: str):
    ...
    if intent in {"FIGURE_QUERY", "TABLE_QUERY", "PAGE_QUERY"}:
        ...
        if intent in {"FIGURE_QUERY", "TABLE_QUERY"} and (
            not target_id or _is_ambiguous_reference()
        ):
        ...
        for chunk_id, chunk_data in metadata.items():
            if intent == "FIGURE_QUERY":
                ...
                ok = (
                    ...
                    and chunk_data.get("page_number") == target_page
                )
        ...
        if intent in {"FIGURE_QUERY", "TABLE_QUERY"}:
            ...
            matched = [
                m for m in matched if not _is_list_index_page(m.get("text", ""), intent)
            ]
            if not matched:
                return [], strict_ref_error
            ...
            if ref:
                ...
                def _table_score(item: dict) -> int:
                    ...
                    multi_col_lines = sum(
                        1
                        for ln in lines
                        if re.search(r"\s{2,}", ln) or "|" in ln or "\t" in ln
                    )
                    return class_lines * 10 + multi_col_lines + min(len(txt) // 200, 20)
...
async def ask_question(request: QuestionRequest):
    ...
    if intent_name in ("GENERAL_QUERY", "SECTION_QUERY"):
        distances = [
            c.get("distance") for c in chunks_data if c.get("distance") is not None
        ]
        if distances:
    ...
    for chunk_info in chunks_data:
        ...
        if intent == "TABLE_QUERY" or comparison_kind == "TABLE_QUERY":
            ...
            intent == "FIGURE_QUERY" or comparison_kind == "FIGURE_QUERY"
        ) and image_path:
            ui_source_type = "IMAGE"
        else:
    ...
    used_explicit_filtering = intent_info.get("intent") in {
        "FIGURE_QUERY",
        "TABLE_QUERY",
        "PAGE_QUERY",
    }
...
"""
    ...
    if intent_info.get("intent") == INTENT_COMPARISON:
        ...
        if any(
            ((s.source_type or s.source) in {"OCR", "IMAGE"}) for s in sources_list[:2]
        ):
        ...
        exact_match = intent_info.get("intent") in {
            "FIGURE_QUERY",
            "TABLE_QUERY",
        } and bool(intent_info.get("target_id"))
    ...
    def is_source_relevant(answer_text: str, source_text: str) -> bool:
        ...
        answer_words = set(w.lower() for w in answer_text.split() if len(w) > 4)
    ...
    for source in sources_list:
        ...
        if (
            intent_info.get("intent") == "FIGURE_QUERY"
            and intent_info.get("target_id")
            and source.page_number is not None
        ):
            display_text = (
                f"{intent_info['target_id']} — Page {source.page_number}\n{source.text}"
            )
    ...
    contextual_sources = (
        _build_contextual_sources(final_sources, intent_info)
        if include_context
        else None
    )
